...
2. **Execute & Track (MANDATORY)**:
   ...
   - **Track Efficacy**: If you retrieve and successfully apply an existing lesson node to solve a bug or write code, you MUST increment its success count to track its efficacy (refer to [synapse-memory](../skills/synapse-memory/SKILL.md#portal-efficacy-workflow) for exact instructions and commands).
...
3. **Finalize (Memory Persistence - MANDATORY)**: At the end of any task, bug fix, story, or feature completion, record insights to the Portal using the `synapse-memory` skill.
   - Refer to [synapse-memory](../skills/synapse-memory/SKILL.md#portal-write-workflow) for strict rules, node type selection criteria (LESSON, FEATURE, CONTEXT), and parameters.
