...
description: "Create implementation plans with adversarial rigor. Includes red-team analysis, risk assessment, and secondary effect analysis."
...
**Fallbasynapse-** Task tools (`TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`) are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for tracking. Plan files remain the source of truth; hydration is an optimization, not a requirement.
