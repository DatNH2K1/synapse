...
description: 'Analyzes current state and user query to answer Synapse questions or recommend the next skill(s) to use. Use when user asks for help, synapse help, what to do next, or what to start with in Synapse.'
---
...
Help the user understand where they are in their Synapse workflow and what to do next, and also answer broader questions when asked that could be augmented with remote sources such as module documentation sources.
...
- **Catalog**: `{project-root}/manifests/synapse-core-system.csv` — assembled manifest of all installed module skills
- **Config**: `.env` at `{project-root}/` — resolve output location variables, provide dynamic configurations like `communication_language` and `project_knowledge`
...
- Skill name in backticks — e.g., `synapse-product-suite`
