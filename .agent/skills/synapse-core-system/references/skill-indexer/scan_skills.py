...
try:
    ...
    raise SystemExit(
        "PyYAML is required. Install with: python3 -m pip install -r scripts/requirements.txt"
    )
...
def scan_skills(base_path: Path) -> List[Dict]:
    ...
    for skill_file in sorted(base_path.rglob('SKILL.md')):
        ...
        try:
            ...
            skill_entry = {
                'name': skill_name,
                'path': str(skill_file.relative_to(Path('skills'))),
...
def main():
    ...
    # Output YAML to local directory
    output_path = Path('skills/synapse-core-system/synapse-skill-manifest.yaml')
