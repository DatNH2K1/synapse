...
Add `STITCH_API_KEY=sk_...` to `~/.env` (or `~/skills/synapse-.env`).
...
```bash
cd ~/skills/synapse-design-suite/scripts && npm install
```

Or run `~/skills/synapse-install.sh` which handles this automatically.
...
Add to `~/.mcp.json` for native design context in Claude Code:
...
1. **Check quota** — Run `stitch-quota.ts check`. If exhausted, suggest `synapse-design-suite` fallback.
...
6. **Implement** — Hand off exported artifacts to implementation skill:
   - `synapse-design-suite` — React/Vue/Svelte components from Tailwind export
   - `synapse-design-suite` — Full page layouts with style guide integration
   - `synapse-ui-styling` — Design token extraction from DESIGN.md
...
- Local tracking via `~/.stitch-quota.json`
- Warns when remaining credits < 20%
- **Fallbasynapse-** When exhausted, use `synapse-design-suite` for text-based design generation
