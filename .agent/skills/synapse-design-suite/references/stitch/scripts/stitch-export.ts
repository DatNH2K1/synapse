...
function generateDesignMd(html: string): string {
  ...
  const componentMatches =
    html.match(
      /<(section|nav|header|footer|main|aside|form|button|input|table|dialog)[^>]*>/gi,
    ) || [];
...
async function main() {
  try {
    ...
    console.log(
      JSON.stringify({ screenId, projectId, format, exported }, null, 2),
    );
  } catch (error: unknown) {
    const err = error as { code?: string; message?: string };
    if (err.code === "NOT_FOUND") {
      console.error(
        `[X] Screen "${screenId}" not found in project "${projectId}".`,
      );
