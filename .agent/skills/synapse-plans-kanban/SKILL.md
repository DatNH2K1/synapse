...
cd skills/synapse-plans-kanban
npm install
```
...
node skills/synapse-plans-kanban/scripts/server.cjs \
  --dir ./plans \
  --open
...
node skills/synapse-plans-kanban/scripts/server.cjs --stop
```

## Skill Invocation

Use `/synapse-kanban` for quick access:

```bash
/synapse-kanban plans/           # View plans dashboard
/synapse-kanban --stop           # Stop kanban server
```
