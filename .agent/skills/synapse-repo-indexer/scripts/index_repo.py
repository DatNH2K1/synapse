...
class BaseLanguageAdapter:
    ...
    def check_environment(self, root: Path, docker_installed: bool, docker_running: bool, errors: list[str]):
        if not self.local_cmd:
            return
        if not is_local_available(self.local_cmd):
            service = find_docker_service_for_language(self.docker_runtime or self.local_cmd, root)
            if not service:
                if not docker_installed:
                    errors.append(f"{self.lang_id.upper()} files present, but local '{self.local_cmd}' is missing and Docker is not installed.")
                elif not docker_running:
                    errors.append(f"{self.lang_id.upper()} files present, but local '{self.local_cmd}' is missing and Docker is not running (please start Docker).")
                else:
                    errors.append(f"{self.lang_id.upper()} files present, but local '{self.local_cmd}' is missing and no active Docker container provides '{self.local_cmd}'.")

    def run_parser_cmd(self, root_dir: Path, file_path: Path, local_cmd_args: list[str], docker_cmd_args: list[str]) -> tuple[list[dict], list[dict]]:
        exports = []
        imports = []
        try:
            file_content = file_path.read_text(encoding="utf-8", errors="ignore")
            if is_local_available(self.local_cmd):
                res = subprocess.run(local_cmd_args, capture_output=True, text=True, timeout=5)
                if res.returncode == 0:
                    data = json.loads(res.stdout)
                    exports = data.get("exports", [])
                    imports = data.get("imports", [])
            else:
                service = find_docker_service_for_language(self.docker_runtime or self.local_cmd, root_dir)
                if service:
                    res = subprocess.run(
    ...
    def run_parser(self, root_dir: Path, file_path: Path) -> tuple[list[dict], list[dict]]:
        return [], []
    ...
    def run_dep_command(self, root: Path, command_type: str):
        ...
        if is_local_available(local_bin):
            print(f"Running dependency {command_type} command locally for {self.lang_id}: {' '.join(cmd_local)}")
            subprocess.run(cmd_local, cwd=root)
        elif service:
            print(f"Running dependency {command_type} command inside Docker service '{service}' for {self.lang_id}: {' '.join(cmd_docker)}")
            subprocess.run(["docker", "compose", "exec", "-T", service] + cmd_docker, cwd=root)
        else:
            print(f"Warning: Neither local '{local_bin}' nor running Docker '{self.docker_runtime}' service found. Cannot run dependency {command_type} command for {self.lang_id}.")
    ...
    def resolve_imports(self, entry: FileEntry, root: Path, php_exports_map: dict[str, str]):
        pass
...
class JavaScriptAdapter(BaseLanguageAdapter):
    ...
    install_docker = ["npm", "install", "--save-dev", "@babel/parser", "@babel/traverse"]
...
class CSSAdapter(BaseLanguageAdapter):
    ...
    def run_parser(self, root_dir: Path, file_path: Path) -> tuple[list[dict], list[dict]]:
        ...
        try:
            file_content = file_path.read_text(encoding="utf-8", errors="ignore")
            for match in re.finditer(r'@import\s+(?:url\()?[\'"]([^\'"]+)[\'"]\)?\s*;', file_content):
...
def is_ignored(target_path: Path, rules: list[IgnoreRule], is_dir: bool) -> bool:
    ...
    if not is_dir:
        ...
        # Docker files
        if name == "docker-compose.yml" or (name.startswith("docker-compose.") and name.endswith(".yml")):
            return True
        ...
        if name.endswith(".config.js") or name.endswith(".config.ts") or name.endswith(".d.ts"):
            return True
        if name in {"tsconfig.json", "tsconfig.node.json", "package.json", "package-lock.json"}:
            return True
    ...
    for c in candidates:
        for s in services:
            if c in s.lower():
                ...
                res = subprocess.run(["docker", "compose", "exec", "-T", s, "which", cmd], capture_output=True, cwd=root)
                if res.returncode == 0:
                    return s
...
def run_parser_adapter(root_dir: Path, file_path: Path) -> tuple[list[dict], list[dict]]:
...
def build_index(root: Path, max_files: int, max_depth: int | None, langs: set[str]) -> list[FileEntry]:
    ...
    for entry in entries:
        entry.imports = [imp for imp in entry.imports if imp.get("from") in scanned_paths]
...
def sync_to_portal(entries: list[FileEntry], repo_name: str) -> bool:
    port = os.getenv("SYNAPSE_PORTAL_PORT", "3100")
    url = f"http://localhost:{port}/api/indexer/sync"
...
def main() -> int:
    ...
    parser.add_argument("--parse", type=str, help="Parse a single file and output its exports/imports JSON")
    parser.add_argument("--path", type=str, default=".", help="Path to the repository to index (default: current directory)")
    parser.add_argument("--repo", type=str, help="Repository name (defaults to folder name of scanned path)")
    ...
    try:
        ...
        if sync_success:
            print(f"Successfully synced repository '{repo_name}' AST dependency graph to Synapse Portal PostgreSQL!")
        else:
            print("Warning: Synapse Portal sync failed. Check if Synapse Portal dev server is running.")
    finally:
