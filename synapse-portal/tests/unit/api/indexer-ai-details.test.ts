...
describe("API Indexer AI Details Route", () => {
  ...
  it("should return 404 if file is not found in database", async () => {
    ...
    const req = new Request("http://localhost/api/indexer/ai/details?file=unknown.ts");
    const response = await GET(req);
    const data = await response.json();
  ...
  it("should return details for file on success", async () => {
    ...
    const req = new Request("http://localhost/api/indexer/ai/details?file=src/index.ts");
    const response = await GET(req);
    const data = await response.json();

    expect(response.status).toBe(200);
    expect(data.success).toBe(true);
    expect(data.symbols).toEqual([{ name: "MyClass", kind: "class", range: "1:0" }]);
